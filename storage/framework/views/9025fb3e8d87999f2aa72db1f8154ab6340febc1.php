<?php
$posts ??= [];
$totalPosts ??= 0;
?>
<?php if(!empty($posts) && $totalPosts > 0): ?>
	<?php
	foreach($posts as $key => $post):
		// Main Picture
		$pictures = (array)data_get($post, 'pictures');
		$defaultImg = config('larapen.core.picture.default');
		$postImg = data_get($pictures, '0.filename', $defaultImg);
	?>
	<div class="item-list">
		<?php if(data_get($post, 'featured') == 1): ?>
			<?php if(!empty(data_get($post, 'latestPayment.package'))): ?>
				<?php if(data_get($post, 'latestPayment.package.ribbon') != ''): ?>
					<div class="ribbon-horizontal <?php echo e(data_get($post, 'latestPayment.package.ribbon')); ?>">
						<span><?php echo e(data_get($post, 'latestPayment.package.short_name')); ?></span>
					</div>
				<?php endif; ?>
			<?php endif; ?>
		<?php endif; ?>
		
		<div class="row">
			<div class="col-sm-2 col-12 no-padding photobox">
				<div class="add-image">
					<span class="photo-count">
						<i class="fa fa-camera"></i> <?php echo e(count($pictures)); ?>

					</span>
					<a href="<?php echo e(\App\Helpers\UrlGen::post($post)); ?>">
						<?php echo imgTag($postImg, 'medium', ['class' => 'lazyload thumbnail no-margin', 'alt' => data_get($post, 'title')]); ?>

					</a>
				</div>
			</div>
			
			<div class="col-sm-7 col-12 add-desc-box">
				<div class="items-details">
					<h5 class="add-title">
						<a href="<?php echo e(\App\Helpers\UrlGen::post($post)); ?>"><?php echo e(str(data_get($post, 'title'))->limit(70)); ?></a>
					</h5>
					
					<span class="info-row">
						<?php if(config('settings.single.show_listing_types')): ?>
							<?php if(!empty(data_get($post, 'postType'))): ?>
								<span class="add-type business-posts"
									  data-bs-toggle="tooltip"
									  data-bs-placement="bottom"
									  title="<?php echo e(data_get($post, 'postType.name')); ?>"
								>
									<?php echo e(strtoupper(mb_substr(data_get($post, 'postType.name'), 0, 1))); ?>

								</span>&nbsp;
							<?php endif; ?>
						<?php endif; ?>
						<?php if(!config('settings.list.hide_dates')): ?>
							<span class="date"<?php echo (config('lang.direction')=='rtl') ? ' dir="rtl"' : ''; ?>>
								<i class="far fa-clock"></i> <?php echo data_get($post, 'created_at_formatted'); ?>

							</span>
						<?php endif; ?>
						<span class="category"<?php echo (config('lang.direction')=='rtl') ? ' dir="rtl"' : ''; ?>>
							<i class="bi bi-folder"></i>&nbsp;
							<?php if(!empty(data_get($post, 'category.parent'))): ?>
								<a href="<?php echo \App\Helpers\UrlGen::category(data_get($post, 'category.parent'), null, $city ?? null); ?>" class="info-link">
									<?php echo e(data_get($post, 'category.parent.name')); ?>

								</a>&nbsp;&raquo;&nbsp;
							<?php endif; ?>
							<a href="<?php echo \App\Helpers\UrlGen::category(data_get($post, 'category'), null, $city ?? null); ?>" class="info-link">
								<?php echo e(data_get($post, 'category.name')); ?>

							</a>
						</span>
						<span class="item-location"<?php echo (config('lang.direction')=='rtl') ? ' dir="rtl"' : ''; ?>>
							<i class="bi bi-geo-alt"></i>&nbsp;
							<a href="<?php echo \App\Helpers\UrlGen::city(data_get($post, 'city'), null, $cat ?? null); ?>" class="info-link">
								<?php echo e(data_get($post, 'city.name')); ?>

							</a> <?php echo e((!empty(data_get($post, 'distance'))) ? '- ' . round(data_get($post, 'distance'), 2) . getDistanceUnit() : ''); ?>

						</span>
					</span>
					
					<?php if(config('plugins.reviews.installed')): ?>
						<?php if(view()->exists('reviews::ratings-list')): ?>
							<?php echo $__env->make('reviews::ratings-list', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
						<?php endif; ?>
					<?php endif; ?>
				</div>
			</div>
			
			<div class="col-sm-3 col-12 text-end price-box" style="white-space: nowrap;">
				<div class="row w-100">
					<div class="col-12 m-0 p-0 d-flex justify-content-end">
						<h2 class="item-price">
							<?php echo getPriceInfo(data_get($post, 'price'), data_get($post, 'category.type') ?? null); ?>

						</h2>
					</div>
					<div class="col-12 m-0 p-0 d-flex justify-content-end">
						<?php if(!empty(data_get($post, 'latestPayment.package'))): ?>
							<?php if(data_get($post, 'latestPayment.package.has_badge') == 1): ?>
								<a class="btn btn-danger btn-sm make-favorite">
									<i class="fa fa-certificate"></i> <span><?php echo e(data_get($post, 'latestPayment.package.short_name')); ?></span>
								</a>&nbsp;
							<?php endif; ?>
						<?php endif; ?>
						<?php if(!empty(data_get($post, 'savedByLoggedUser'))): ?>
							<a class="btn btn-success btn-sm make-favorite" id="<?php echo e(data_get($post, 'id')); ?>">
								<i class="fas fa-bookmark"></i> <span><?php echo e(t('Saved')); ?></span>
							</a>
						<?php else: ?>
							<a class="btn btn-default btn-sm make-favorite" id="<?php echo e(data_get($post, 'id')); ?>">
								<i class="fas fa-bookmark"></i> <span><?php echo e(t('Save')); ?></span>
							</a>
						<?php endif; ?>
					</div>
				</div>
			</div>
		</div>
	</div>
	<?php endforeach; ?>
<?php else: ?>
	<div class="p-4 w-100">
		<?php echo e(t('no_result_refine_your_search')); ?>

	</div>
<?php endif; ?>

<?php $__env->startSection('after_scripts'); ?>
	<?php echo \Illuminate\View\Factory::parentPlaceholder('after_scripts'); ?>
	<script>
		
		var lang = {
			labelSavePostSave: "<?php echo t('Save listing'); ?>",
			labelSavePostRemove: "<?php echo t('Remove favorite'); ?>",
			loginToSavePost: "<?php echo t('Please log in to save the Listings'); ?>",
			loginToSaveSearch: "<?php echo t('Please log in to save your search'); ?>"
		};
	</script>
<?php $__env->stopSection(); ?>
<?php /**PATH G:\xampp\htdocs\classified\resources\views/search/inc/posts/template/grid.blade.php ENDPATH**/ ?>